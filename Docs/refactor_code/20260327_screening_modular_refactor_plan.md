...
**修正**：將 `fetch_stock_prices()`、`get_stocks_by_markets()` 移至 `screening/service.py`，`pattern/routes.py` 改為 `from ..service import fetch_stock_prices, get_stocks_by_markets`。
...
### Step 2 — app/ 下子目錄改小寫
7. `feature/Screening/` → `feature/screening/` 等全部重命名
8. 更新所有 Python import 路徑（`app.py`、`feature/__init__.py`、各 `routes.py`、`data_management/` 下所有檔案）
9. 更新所有 HTML 模板路徑
10. ✅ 驗證：所有頁面正常渲染
