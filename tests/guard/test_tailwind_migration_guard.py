"""tests/guard/test_tailwind_migration_guard.py — CSS/JS 遷移守護測試"""
import pytest

pytestmark = pytest.mark.guard


def test_chart_modal_css_consolidated_import(read_project_file):
    content = read_project_file("app/static/css/input.css")
...
def test_old_chart_modal_css_files_deleted(file_exists_checker):
    deleted_files = [
    ...
    for file_path in deleted_files:
        assert not file_exists_checker(file_path), f"Expected deleted file still exists: {file_path}"


def test_screening_js_has_no_style_display_mutation(project_root):
    screening_root = project_root / "app/feature/screening"
    ...
    for js_file in js_files:
        ...
        if "style.display" in text:
            offenders.append(str(js_file.relative_to(project_root)).replace("\\", "/"))

    assert not offenders, f"Found style.display usage in: {offenders}"


def test_strategy_buttons_have_spacing_class(read_project_file):
    ui = read_project_file("app/feature/screening/components/strategy_manager/templates/ui.html")
    assert 'class="button-group flex flex-col gap-2"' in ui


def test_stop_dialog_utility_migration_applied(read_project_file):
    dialog_tpl = read_project_file("app/feature/screening/components/progress_area/templates/ui.html")
    screening_css = read_project_file("app/feature/screening/screening.css")
...
def test_stock_list_state_class_contract(read_project_file):
    screening_css = read_project_file("app/feature/screening/screening.css")
    progress_js = read_project_file("app/feature/screening/components/progress_area/progress_area.js")
    results_js = read_project_file("app/feature/screening/components/results_table/results_table.js")
    screening_js = read_project_file("app/feature/screening/screening.js")
...
def test_indicator_modules_no_inline_style_templates(read_project_file):
    module_files = [
...
def test_screening_fragment_layout_is_class_based(read_project_file):
    fragment = read_project_file("app/feature/screening/screening_fragment.html")
...
def test_chart_fullscreen_class_contract_is_class_based(read_project_file):
    chart_css = read_project_file("app/feature/screening/chart/kline_viewer/chart-area.css")
    screening_js = read_project_file("app/feature/screening/screening.js")
...
def test_chart_modal_template_uses_semantic_classes(read_project_file):
    template = read_project_file("app/feature/screening/chart/chart_management/chart_settings_modal_template.js")
    modal_css = read_project_file("app/feature/screening/chart/chart_management/chart-modal.css")
...
def test_chart_modal_presentational_inline_styles_removed(read_project_file):
    indicator_tab = read_project_file("app/feature/screening/indicators/indicator_settings_tab.js")
    pattern_tab = read_project_file("app/feature/screening/pattern/pattern_settings_tab.js")
...
def test_chart_modal_state_class_guard(read_project_file):
    modal_css = read_project_file("app/feature/screening/chart/chart_management/chart-modal.css")
    modal_js = read_project_file("app/feature/screening/chart/chart_management/chart_settings_modal.js")
...
def test_indicator_manager_summary_state_class_contract(read_project_file):
    manager_js = read_project_file("app/feature/screening/indicators/indicator_manager.js")
    screening_css = read_project_file("app/feature/screening/screening.css")
...
def test_risk_management_no_broad_input_override(read_project_file):
    css = read_project_file("app/feature/risk_management/risk_management.css")
...
def test_risk_management_templates_are_class_based(read_project_file):
    template_files = [
    ...
    fragment = read_project_file("app/feature/risk_management/risk_management_fragment.html")
    assert 'class="page-content page-content--fill active" id="risk-page"' in fragment


def test_risk_portfolio_block_static_inline_styles_removed(read_project_file):
    js = read_project_file("app/feature/risk_management/components/portfolio/portfolio_block.js")
...
def test_input_css_import_convergence_wave6(read_project_file):
    input_css = read_project_file("app/static/css/input.css")
...
def test_global_css_wave1_convergence_contract(read_project_file):
    layout_css = read_project_file("app/static/css/layout.css")
    components_css = read_project_file("app/static/css/components.css")
    tabs_css = read_project_file("app/static/css/tabs.css")
    animations_css = read_project_file("app/static/css/animations.css")
...
def test_risk_portfolio_centered_layout_contract(read_project_file):
    css = read_project_file("app/feature/risk_management/risk_management.css")
    js = read_project_file("app/feature/risk_management/components/portfolio/portfolio_block.js")
...
def test_risk_portfolio_column_separator_contract(read_project_file):
    css = read_project_file("app/feature/risk_management/risk_management.css")
...
def test_risk_params_local_storage_and_formatting_contract(read_project_file):
    params_js = read_project_file("app/feature/risk_management/components/params/params_block.js")
